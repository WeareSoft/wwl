# Delf

# 2020-02-16
- bolling
   - step2 스코어링 구현하기
   - 입출력 분리 조금 더 고민해보기
   - 테스트 작성하기
- snak
   - 프로젝트 구조 조정
   - news, picks 벌크 업로드 수정
   - JPA 최적화
- 자소서

# 2020-02-23
- wwl
  - 
- snak
  - JPA bulk (batch insert) 공부 및 구현
  - slack web-hook, aws lambda로 snak 뉴스봇 조사 및 구현
- 포트폴리오 수정
- 알고리즘 (최소 두 문제)

# 2020-02-09
- snak
  - 무한스크롤 마무리
  - JPA 최적화(페이징, 트랜잭션)
- 자소서
- 알고리즘 (최소 두 문제)

- snak
  - 무한스크롤 마무리
  - bulk upload 구현
- 알고리즘 (최소 두 문제)

- 로또 구현하기 (3/4)
  - (계속)
- snak
  - 무한스크롤 마무리
  - bulk upload 구현
- 알고리즘 (최소 두 문제)

# 2020-02-02
- 로또 구현하기 (3/4)
  - 예외처리하기
  - 숫자 결정 정책 적용
  - 테스트 작성하기 
- snak
  - picks API 구현하기
  - news 무한스크롤 구현하기

# 2020-01-19
- 로또 구현하기 (2/4)
  - 로또 step 1 변경하기 (커밋 정리, 로직 다듬기, 리뷰 기반 보완)
  - step2 진행하기
- 이력서 및 포트폴리오 보강
- 코딩 테스트 준비(최소 3회)

# 2020-01-12
- 로또 구현하기 (1/4) - [링크](https://github.com/WeareSoft/java-lotto)
- 이력서 및 포트폴리오 보강
- snak
  - picks API 구현 및 이슈 해결
  - 테스트 junit5 기능 추가

# 2019-12-22
- snack-news
- 면접 준비
- 코딩 테스트
- 프로젝트 트러블 슈팅 블로그 작성

# 2019-12-15
- snack-news
  - advice 버그 해결 
  - junit5
  - 리펙토링
  - (시간나면 QueryDSL 보기)
- 코딩테스트 대비 알고리즘 문제 풀고 유형 정리

# 2019-12-08
- snack-news
  - 구조 정리(예외처리 정리, 패키지 변경)
- 이력서 정리하기
  - snak 재정리
- 면접 준비하기
  - 개발자로써 성장 배경과 자신의 생각 등을 정리
  - 테크면접도
  - 추가로 블로그 다시 시작하기

# 2019-12-01
- snack-news
  - 구조 정리(예외처리 정리, 패키지 변경)
- 블로그 글 작성
- 계획 구체화

# 2019-11-24
- snack-news
  - 구조 정리(예외처리 정리, 패키지 변경)
  - API 완성
- 면접 준비
    블로그 글 작성

# 2019-11-17
- snack-news
  - 구조 정리
  - 테스트 변경
- 면접 준비
- 오브젝트 6장

# 2019-11-10
- snack-news
- 면접 준비
- 오브젝트 5장 및 4장 이슈 정리하기

# 2019-11-03
- snack-news
  - 어드민 api 추가
- 데뷰 체험 정리
- 포드폴리오 보수
- 오브젝트 4장 읽기
- 카카오 알고리즘 문제 풀기 및 풀이 보기 최소 한 문제. 최대 2문제

# 2019-10-27
- snack-news
  - 배포 사이클 복구 및 정리
  - 포트폴리오 수정 및 추가
- 필기시험 준비
- 카카오 알고리즘 문제 풀기 및 풀이 보기 최소 한 문제. 최대 2문제
- 오브젝트

# 2019-10-20
- snack-news
  - spring security 적용
- 알고리즘 문제
  - 매일프로그래밍 발, 두 문제

# 2019-10-13
- snack-news
  - invailid host header 문제 해결
- snack-news로 포트폴리오 보강
- 알고리즘 문제
  - 매일프로그래밍 발, 두 문제

# 2019-09-29
- snack-news
  - admin 배포 및 api 수정
- 코딩 테스트 및 

# 2019-09-22
- snack-news
  - admin 배포 및 api 수정
- 단위테스트 스터디
  - 로또 구현 및 테스트 코드 작성
- 알고리즘 문제 풀기
  - 매일프로그래밍 

# 2019-09-15
- snack-news
    - admin 배포
- 단위테스트 이번주 분량 공부
  - TDD로 문자열 계산기$$
- 알고리즘 문제 풀기
  - 카카오 블라인드 테스트 문제 다시보기
  - 매일프로그래밍 

# 2019-09-08
- snack-news
    - QuerryDSL 추가
    - 테스트 개선# > < *
- 블로그 포스트 작성
  - 컨트롤러 예외처리
- 단위테스트 이번주 분량 공부
  - TDD로 문자열 계산기
- 알고리즘 문제 풀기
c
# 2019-09-01
- snack-news
- 블로그 포스트 작성
- 단위테스트 이번주 분량 공부
- 알고리즘 문제 풀기

# 2019-08-25
- 블로그 포스트 작성
- 단위테스트 이번주 분량 공부
- 포트폴리오에 snack-news 추가 및 정리
- 매일프로그래밍 구독 알고리즘 문제 풀기

# 2019-08-18
- snack-news
    - QuerryDSL 추가
    - DB 독립적 테스트로 변경
- 단위테스트 이번주 분량 공부
- 포트폴리오
- 알고리즘 문제 풀기(최소 3문제)

# 2019-08-11
- snack-news
    - 테스트 mock 추가
- 스터디 분량 공부하기
- 포트폴리오
- 알고리즘 문제 풀기(최소 3문제)

# 2019-08-04
- 부스트캠프 선수과목 수강하기
- 스터디 분량 공부하기
  - 이번 주 내로 mock 테스트에 추가하기
- 포트폴리오, 이력서 정리
- 알고리즘 문제 풀기(최소 3문제)

# 2019-07-28
- 스터디 분량 공부하기
  - 추가로 mock 
- snack-news
  - 모델 구조 변경
  - response 맞추기
  - 테스트 변경
- 포트폴리오, 이력서 정리

# 2019-07-21 
- 스터디 분량 공부하기
  - 일단 1~2까지 전부
- 포트폴리오, 이력서 정리
- snack-news
  - response 맞추기
  - JPA 동적 쿼리 적용하기 

# 2019-06-30
- 스터디 회고 정리 및 다음 스터디 주제 생각하기
- 블로그
  - 주 최소 2회 포스트 작성하기
- 알고리즘 문제 풀기
  - 코딩테스트 준비
  - SCPC 문제 풀어보기 (고민 많이하지 말고 적당히 풀고 유형 알아보기)
- 포트폴리오 수정하기
- snack-news
    - JPA 동적 쿼리 적용하기 
    - API 완성하고 문서화하기

# 2019-06-23
- 이펙티브 자바 아이템 39~43
- 블로그
  - 주 최소 2회 포스트 작성하기
- 알고리즘 문제 풀기
  - 코딩테스트 준비
  - daily programming, 해커랭크, 코딜리티 중 최소 3문제 이상
- 포트폴리오 수정하기
- snack-news
    - 이번 달 내로 배포 목표
    - 문제 해결 기록하기

# 2019-06-16
- 이펙티브 자바 아이템 88~90, 26~38
- 블로그!!!!
  - 주 최소 2회 포스트 작성하기
  - 이미지 개선하기
- 알고리즘 문제 풀기
  - daily programming, 해커랭크, 코딜리티 중 최소 3문제 이상
- 포트폴리오 수정하기
- snack-news 프로젝트 기록

# 2019-06-09
- 이펙티브 자바 아이템 82~87
- 블로그
  - 주 최소 2회 포스트 작성하기
  - 이미지 개선하기
- 알고리즘 문제 풀기
  - daily programming, 해커랭크, 코딜리티 중 최소 3문제 이상
- todo list 프로젝트 이어하기

# 2019-06-02
- 이펙티브 자바 아이템 76~81
- 알고리즘 문제 풀기
  - daily programming, 해커랭크, 코딜리티 중 최소 3문제 이상
- todo list 프로젝트 이어하기
- 
# 2019-05-26
- 정신 차리기
- 이펙티브 자바 아이템 70~75
- 블로그
  - 주 최소 2회 포스트 작성하기
- 알고리즘 문제 풀기
  - daily programming, 해커랭크, 코딜리티 중 최소 3문제 이상

# 2019-05-19
- 이펙티브 자바 아이템 64~69
- 블로그
  - 블로그 정리하기
  - iterator와 threadlocal 정리
- 알고리즘 문제 풀기
  - daily programming, 해커랭크, 코딜리티 중 최소 3문제 이상

# 2019-05-12
- 이펙티브 자바 아이템 58~63
- 알고리즘 문제 풀기
  - daily programming, 해커랭크, 코딜리티 중 최소 3문제 이상
- 책 읽고 정리: 1만 시간의 재발견

# 2019-05-05
- 이펙트브 아이템 52~57
- todo list 프로젝트 이어하기
- 알고리즘 문제 풀기
- 책 읽고 정리: 1만 시간의 재발견

# 2019-04-28
- 이펙트브 아이템 47~51
- 책 읽고 정리: 1만 시간의 재발견
- 알고리즘 문제 풀기

# 2019-04-21
- 우아한 테크코스 오프라인 코딩테스트 준비
  - 해왔던 미션 피드백 확인하기
- 이펙티브 자바 못했던 부분 정리하기
  - 최소 하루 하나 이상
- 알고리즘 문제풀기
  - daily programming, 해커랭크, 코딜리티 중 최소 3문제 이상

# 2019-04-14
 우아한 테크코스 미션 구현
  - 구현 과정 혹은 느낀점 정리
  - 테스트 코드 짜기
- 이펙티브 자바 못했던 부분 정리하기
  - 최소 하루 하나 이상
- 알고리즘 문제풀기
  - daily programming, 해커랭크, 코딜리티 중 최소 3문제 이상

## 2019-04-07
- 우아한 테크코스 미션 구현
  - 구현 과정 혹은 느낀점 정리
  - README.md를 조금 더 관리하기
- 이펙티브 자바 못했던 부분 정리하기
  - 최소 하루 하나 이상
- 알고리즘 문제풀기
  - daily programming, 해커랭크, 코딜리티 중 최소 3문제 이상
- Todo list 앱 어플리케이션 제작 계획

## 2019-03-31
- 우아한 테크코스 미션 구현
  - 구현 과정 혹은 느낀점 정리
- 이펙티브 자바 읽고 정리
  - 아이템 53~58
- 블로그
  - intelliJ 활용
- tech-interview
  - 기존 내용 검수

## 2019-03-24
- 이펙티브 자바 읽고 정리
  - 아이템 46~52
- 블로그
  - intelliJ 활용
- tech-interview
  - 기존 내용 검수
- devist
  - DTO 수정
  - 테스트 코드 개선

## 2019-03-17
- 이펙티브 자바 읽고 정리
  - 아이템 41~46
  - 람다식과 스트림에 대한 "기본 개념"과 
- tech-interview
  - 기존 내용 검수
- 블로그
  - 제네릭
  - intelliJ 활용
- devist 리펙토링
  - UI 개선과 DTO 수정
-  해커랭크 + 코딜리티 최소 세문제

## 2019-03-10
- 이펙티브 자바 읽고 정리
  - 아이템 41~46
  - 아이템 39 다시보기
- tech-interview
  - 기존 내용 검수
- 블로그 올리기
- devist 배포
  - 기능 추가 및 리팩토링하기
  - 다듬어서 빌드 가능하게 하기
- 해커랭크 + 코딜리티 최소 세문제

## 2019-03-03
- 이펙티브 자바 읽고 정리
  - 아이템 38~40 추가 
- 코딜리티, 해커랭크 문제풀면서 알고리즘 공부
  - 코딜리티 최소 세 문제
  - 해커랭크 최소 두 문제(java 30days 제외)
- devist 배포

## 2019-02-25
- 이펙티브 자바 읽고 정리
  - 아이템 31~37
- 코딜리티, 해커랭크 문제풀면서 알고리즘 공부
  - 코딜리티 최소 세 문제
  - 해커랭크 최소 두 문제(java 30days 제외)
- tech-interview
  - 한번 훑어보면서 공부하기, 틀린 곳 수정하기
  - 동기화와 비동기화 
- devist
- 블로그 포스팅
  - devist
  - 이펙티브 자바

## 2019-02-17
- 이펙티브 자바 읽고 정리
  - 아이템 25~30
- 코딜리티 문제풀기
- tech-interview 업로드
  - 동기화와 비동기화
- devist
  - 막혀있던 JPA와 모델 상속 문제 해결
  - 테스트 문제 해결
  - 진행 계획 다시 짜기

## 2019-02-10
- 이펙티브 자바 읽고 정리(추가)
  - 아이템 22~24 추기
- 코딜리티 문제풀기
  - 최소 세 문제, 목표 네 문제
- tech-interview 업로드
  - 동기화와 비동기화
  - https://github.com/WeareSoft/tech-interview/issues/25
- devist 재시작

## 2019-02-03
- 이펙티브 자바 읽고 정리하기
  - 아이템 17~21
  - 20, 21을 중점적으로
  - 이전에 정리 못했던 아이템 정리하기
- devist
  - 이번달 내로 배포 진행 계속
  - 인터페이스 적용하여 구조 변경
  - mock을 이용한 다른 자원에 비종속적인 테스트 실현
- 코딜리티 문제풀기
  - 최소 두 문제, 목표 세 문제

## 2019-01-27
- 이펙티브 자바 읽고 정리하기
  - 아이템 10~16
- Devist 프로젝트 이번 달 내로 배포 목표
- 금요일까지 알고리즘 최소 세 문제정도 풀기
- tech-interview 작성

## 2019-01-20
- 이펙티브 자바 하루에 하나씩 읽고 정리하기
  - 아이템5, 아이템7 공부하기
- Devist 프로젝트 진행
- 부스트코스 강좌 수강
  - 프론트에 대한 부족함을 느낌 - 윤지수님의 HTML&CSS 강좌 수강
- tech-interview 하나 올리기
  - JDBC

## 2018-12-23
- devist 프로젝트 진행
  - DB 설계 완료, 구현
- spring-store 프로젝트 진행
- 연간 회고
- tech interview
  - JDBC란
- 연간회고

## 2018-12-16
- 클라우드 컴퓨팅 기말고사
- T-acamedy 세미나
- 웹프레임워크2 동영상 학습
  - spring-store 개인 프로젝트 (본격적으로) 시작
  - devist 프로젝트 계속
  - 관련 내용 포스팅
- tech interview 
  - [ORM이란](https://github.com/WeareSoft/tech-interview/blob/master/contents/db.md#orm이란)
- 연간회고하며 돌아보기

## 2018-12-09
- 클라우드 컴퓨팅 과제 및 발표 준비 [#](https://github.com/WeareSoft/wwl/blob/master/delf/contents/aws-webserver-rds.md)
- 프로젝트
    - Spring MVC를 이용한 'spring-store(가칭)' 프로젝트 진행 및 공부
        - 선행사항은 웹프레임워크 동영상 보는 것
    - Spring Boot를 이용한 '반복 Toto 관리 도우미(가칭)' 프로젝트 진행 및 공부
- 연간회고하며 돌아보기
- 면접 [#] (/contents/Summary_20181209.md)
- 기타 사강 과제, 시험, 토론 마무리

## 2018-12-02
- 면접 [#](/contents/Summary_20181202.md)
- 김남윤 교수님 웹프레임워크 동영상 보기(일단 시청)
    - 적어도 1은 다 보기
- 사강 과제, 퀴즈, 기말고사...
- 회고 [#](../feedback/181202.md#delf)

## 2018-11-25
- Spring 공부 및 프로젝트 진행
    - 게시판, todo 관리 사이트 생각 중
    - 관련된 것 공부, 정리, 블로깅
- tech-interview
    - [java의 main 메서드가 static인 이유](https://github.com/WeareSoft/tech-interview/issues/14)
    - (+욕심) 하나 더 올리기
- 내가 진짜로 가고싶은 기업 생각해보기
    - 업무, 사람, 개발문화, 연봉, 복지 등등 여러 것들을 고려해서
- 회고 [#](../feedback/181125.md#delf)

## 2018-11-18
- 동북아 공개 sw활성화 포럼 참가
- Spring 공부하기

## 2018-11-11
- 블로그
    - Oauth
- Spring 공부하기
    - IntellJ + Spring
    - eStore > 게시판(더 효과적인 과정이 있다면...)
- API 사용방법에 대해서 공부하기
    - 쓸만한 내용이라면 포스팅
    - Oauth랑 관련해서 알아보기
- 여행 가기 전에 과제 마무리
- WAS 로고 생각해보기

## 2018-11-05
- 클라우드 컴퓨팅 중간고사
- CAMPUS HACKDAY 작성
- 블로깅 및 개념 공부
- 알고리즘 문제 풀이

## 2018-10-28
- 블로그 포스트 정리
    - Oauth, String
- 2018 EDAILY Coding Challenge 참가 [#](https://github.com/Delf-Lee/Daily-Programing/tree/master/codingtest/edailycoding2018challange)
    - 난이도 자체는 많이 어렵지 않은 듯함
    - 120문제(20문제?) 48시간이라는 특이한 형식...
        - 숙지하지 않은 상태에서 문제를 풀어서 다 못풀었음
- 그 밖의 알고리즘 문제 풀이 [#](https://github.com/Delf-Lee/Daily-Programing/commits/master)

## 2018-10-21
- NHN pre-test 2차
    - 기본 프로그래밍 문제(함수형 프로그래밍, 파이썬)
    - 운영체제(부동소수점, 스케쥴링, 가상메모리, 페이징)
    - 자료구조/알고리즘(이진트리, 스택, 큐, 시간 복잡도)
    - 네트워크(OSI 7 layer, 라우팅)
    - 데이터베이스(SQL, 트랜잭션)
- 알고리즘 문제 풀이 [#](https://github.com/Delf-Lee/Daily-Programing/commits/master)

## 2018-10-14
- 블로그
    - https://delf-lee.github.io/raid/
    - https://delf-lee.github.io/restful/
    - https://delf-lee.github.io/connection-pool/
- 알고리즘 문제 풀이 [#](https://github.com/Delf-Lee/Daily-Programing/commits/master)
- 적성 준비

## 2018-10-07
- 알고리즘 문제 풀이 [#](https://github.com/Delf-Lee/Daily-Programing/commits/master)
- 면접 이론 공부
    - Connection Pool [#](https://delf-lee.github.io/2018/10/04/connection-pool/)

## 2018-09-30
- 알고리즘 문제 풀이
    - 프로그래머스 - 전화번호부 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/Solution_023.java)
    - 프로그래머스 - 완주하지 못한 선수 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/Solution_022.java)
    - 프로그래머스 - 위장 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/Solution_024.java)
    - 프로그래머스 - 베스트 앨범 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/Solution_025.java)
    - 프로그래머스 - 탑 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/Solution_026.java)
    - 프로그래머스 - 다리를 지나는 트릭 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/Solution_027.java)
    - 프로그래머스 - 프린터 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/Solution_027.java)
    - NHN pre-test 01 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/NHN_pretest01.java)
    - NHN pre-test 03 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/NHN_pretest02.java)
    - NHN pre-test 04 (완벽 파악했다 곧 푼다)
    - NHN pre-test 모의1 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/Solution_029.java)
    - NHN pre-test 모의2 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/Solution_030.java)
    - 프로그래머스 - 주식가격 [#](https://github.com/Delf-Lee/Daily-Programing/commit/733e17f3a7e12e3d3eacf4d30884993bf9bcb7d7)
- 자소서 작성
- 포트폴리오 수정

## 2018-09-16
- 알고리즘 문제 풀이
    - 카카오 블라인드 01[#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/dailyprogramming/Solution_021.java)
    - 괄호 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/backjoon/Q10422.java)
    - 단어길이 재기 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/backjoon/Q2743.java)
    - 문자열 분석 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/backjoon/Q10820.java)
    - ROT13 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/backjoon/Q11655.java)
    - 접미사 배열 [#](https://github.com/Delf-Lee/Daily-Programing/blob/master/src/main/java/backjoon/Q11656.java)

## 2018-09-09
### 백준 알고리즘 공부
- pdf자료 기준으로 하루 반~한챕터 끝내기
### 자바 공부
- 이팩티브 자바 2~4장 정리
### 블로그 만들기 - delf-lee.github.io
- 알고리즘 풀이, 개념 공부 하면서 정리하면 좋겠다 싶은거 정리하기.
- 클라우드 컴퓨팅 수업 정리해서 올리기

## 2018-09-02
### 블로그 만들기 - delf-lee.github.io
- 포스팅
### 백준 알고리즘 공부
- 개념 공부
- 백준 알고리즘 repo 파서 풀어보기(기존 DailyProgramming repo랑 통합 생각)
### 자바 공부
- 스프링으로 무언기 만들어보기
- Spring(Web + Java) + DB

## 2018-08-26 [(정리)](https://github.com/WeareSoft/WWL/blob/master/delf/contents/Summary_20180826.md)
### 알고리즘 문제 풀기
#### 백준 알고리즘 공부
### 블로그 만들기
- hexo
- 공부 했던거 포스팅
- 면접 개념

### 스프링 공부
- 공부 방법 찾기

## 2018-08-19[ (정리)](https://github.com/WeareSoft/WWL/blob/master/delf/contents/Summury_20180819.md)
### 카카오 코드페스티벌 예선 문제 2문제 풀기 
### 자바 웹 프로그래밍 Next Step 공부
#### 9장 마무리 - [정리](https://github.com/WeareSoft/WWL/blob/master/delf/contents/JavaWebProgrammingNetxtStep/20180812_Chapter09.md)
- 이 전까지 배웠던 개념 정리 및 시험
- 두 번째 양파껍질 벗길 준비 완료!
#### 10장 - 새로운 MVC 프레임워크 구현을 통한 점진적인 개선 - [정리](https://github.com/WeareSoft/WWL/blob/master/delf/contents/JavaWebProgrammingNetxtStep/20180819_Chapter10.md)
- 애노테이션 설정을 이용한, 컨트롤러 매핑의 자동화
- 다양한 종류의 프레임워크들의 컨트롤러를 **<u>점진적으로</u>** 통합하는 방법

#### 11장 - 의존과계 주입(이하 DI)을 통한 테스트하기 쉬운 코드 만들기 - [정리](https://github.com/WeareSoft/WWL/blob/master/delf/contents/JavaWebProgrammingNetxtStep/20180819_Chapter11.md)

#### 12장 - 세 번째 양파 껍질 벗기기 - [정리]((https://github.com/WeareSoft/WWL/blob/master/delf/contents/JavaWebProgrammingNetxtStep/20180819_Chapter12.md))

## 2018-08-12
 ### 알고리즘 공부
- 카카오 코드페스티벌 예선 문제 풀어보고, 무슨 개념이 적용되었는지 알기
### 자바 웹 프로그래밍 Next Step 공부
#### 8장 학습 - [정리](https://github.com/WeareSoft/WWL/blob/master/delf/contents/JavaWebProgrammingNetxtStep/20180812_Chapter08.md)
- AJAX로 게시판 구현하기, 다른 기술 알아보기
- MVV 프레임워크 요구사항 2단계로 구현하기

#### 9장 학습 - [정리](https://github.com/WeareSoft/WWL/blob/master/delf/contents/JavaWebProgrammingNetxtStep/20180812_Chapter09.md)
- 구현했던 개념들에 대학 복습 및 심화과정 진행

## 2018-08-05
### 알고리즘 공부
- 하루에 2시간(최소 1시간) 백준 알고리즘 개념 보기 or 알고리즘 문제 풀기
- 카카오 페스티벌 예선 준비를 위함
### JWPNS 공부
#### 7장 학습 - [정리](https://github.com/WeareSoft/WWL/blob/master/delf/contents/JavaWebProgrammingNetxtStep/20180805_Chapter07.md)
- 클라이언트 데이터를 DB에 저장
- DAO와 Java의 JDBC 학습
- 깔끔한 코드가 무엇인가
#### 8장 학습
- AJAX로 게시판 구현하기, 다른 기술 알아보기
- MVV 프레임워크 요구사항 2단계로 구현하기

## 2018-07-29
### 학습 도서
- [자바 웹 프로그래밍 Next Step(로드북 출판, 박재성 저)](http://www.aladin.co.kr/shop/wproduct.aspx?ItemId=91501933)

### 5장 정리
- HttpRequestSetvlet, HttpResponseSevlet의 구조 알기.
### [6장 학습](https://github.com/WeareSoft/WWL/blob/master/delf/contents/JavaWebProgrammingNetxtStep/20180729_Chapter06.md)
- HttpRequest & HttpResponse
- Forward & Redirect
- DispatcherServlet 
- Session
- MVC Framework
- ResourceFilter
