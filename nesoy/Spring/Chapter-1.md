...
### 1.7 의존관계 주입(DI)
- Object는 다른 Object에 주입할 수 있는게 아니다.
- DI는 오브젝트 레퍼런스를 외부로부터 제공(주입)받고 이를 통해 여타 오브젝트와 다이내믹하게 의존관계가 만들어지는 것이 핵심이다.


#### 의존관계
- A가 B에 의존하고 있다.
    - B가 변하게 된다면 영향은 A에게 미친다.
- 인터페이스를 통해 의존관계를 제한해주면 그만큼 변경에서 자유로워지는 셈이다.
![No Image](/nesoy/Images/Spring/2.png)

- 모델이나 코드에서 클래스와 인터페이스를 통해 드러나는 의존관계 뿐만 아니라 런타임 시에 오브젝트 사이에서 만들어지는 의존관계도 있다.
- 누가 사용할지 미리 알 수 없다. 즉 실제 사용대상인 오브젝트를 의존 오브젝트(dependent Object)라고 말한다.
